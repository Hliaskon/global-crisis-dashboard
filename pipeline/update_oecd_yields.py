...
# -*- coding: utf-8 -*-
"""
Fetch 10Y government bond yields (OECD/MEI via FRED) with robust fallbacks:
1) FRED JSON API (uses FRED_API_KEY if set)
2) FRED API CSV (file_type=csv)
3) fredgraph.csv (graph download)
Usage:
  python pipeline/update_oecd_yields.py <ISO2> <out_csv>
Example:
...
- OECD long-term yields via FRED: see “Interest Rates: Long-Term Government Bond Yields” (MEI) listings. 
"""
...
API_CSV_URL  = JSON_URL     # same endpoint, choose file_type=csv
GRAPH_CSV_URL= "https://fred.stlouisfed.org/graph/fredgraph.csv"

# Map ISO2 -> FRED series for OECD 10Y yields (monthly main benchmark)
SERIES = {
    "US": "IRLTLT01USM156N",
    "DE": "IRLTLT01DEM156N",
    "FR": "IRLTLT01FRM156N",
...
def _normalize_date_value(df: pd.DataFrame, prefer_series_col: str = None) -> pd.DataFrame:
    ...
    if "value" in df.columns:
        ...
        # fall back to first non-date col
...
def main():
    if len(sys.argv) < 3:
        print("Usage: update_oecd_yields.py <ISO2> <out_csv>")
        sys.exit(2)
